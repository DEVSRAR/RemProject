...
function shenpi(url,token,join_code,code,account_id,f_rom){
	//			token join_code code account_id
//			$('.asdf').val(join_code+','+code+','+account_id+','+f_rom)
			var account_ids=account_id
			var f_roms=f_rom.split(',')[0]
			util.post({
				url: api_www+url,
				data: {
					token: token,
				...
				callback: function(data) {
					if(data.data.clearType==1){
						data.data.clearType="支付宝"
					}else if(data.data.clearType==2){
						data.data.clearType="微信"
					}else if(data.data.clearType==3){
						data.data.clearType="银行卡转账"
					}else if(data.data.clearType==4){
						data.data.clearType="现金"
					}
					if(data.data.approvalPerson.length>1){
						var dates=Date.parse(new Date())
						var strtime = data.data.approvalPerson[data.data.approvalPerson.length-2].create_time
						if(strtime){
						strtime= strtime.split(/[- : \/]/)
						strtime = new Date(strtime[0], strtime[1]-1, strtime[2], strtime[3], strtime[4], strtime[5]);
						var datee = Date.parse(new Date(strtime)); //传入一个时间格式，如果不传入就是获取现在的时间了，这样做不兼容火狐。
						var d=dates-datee
						data.data.approvalPerson[data.data.approvalPerson.length-1].create_time='已等待'+ formatDuring(d)						

						}
					}else{
						if(data.data.approvalPerson.length==0){}else{
							var dates=Date.parse(new Date())
						var strtime = data.data.create_time
						strtime= strtime.split(/[- : \/]/)
						strtime = new Date(strtime[0], strtime[1]-1, strtime[2], strtime[3], strtime[4], strtime[5]);
						var datee = Date.parse(new Date(strtime)); //传入一个时间格式，如果不传入就是获取现在的时间了，这样做不兼容火狐。
						var d=dates-datee
					...
					$('input[name=join_code]').val(join_code)
					console.log(data.data.state)
						//判断发起人
					
						if(data.data.account_id==account_ids && data.data.state==0 && f_roms=='0'){
							$('.cx').show()
							$('.zt').addClass('spz')
						}

						//审批状态
						if(data.data.state==0 &&  f_roms=='-1'){
							//审批中
							$('.en').show()
							$('.zt').addClass('spz')
						...
						for(var i=0;i<data.data.approvalPerson.length;i++){
							if(data.data.approvalPerson[i].id==account_ids){
								$('.zt').addClass('spz')
								$('.ae').show().siblings().hide()
								$('.footer').hide()
							}
						}
						...
						var token, join_code, code, account_id, f_rom
						if(isAndroid) {
							$('.jump').click(function(){
								var store_code=$('input[name=store_code]').val()
								var user_id=$('input[name=user_id]').val()
								var joinCode=$('input[name=join_code]').val()
								window.xmh.jump(user_id,joinCode,store_code)
							})
						} else {
							$('.jump').click(function(){
								var store_code=$('input[name=store_code]').val()
								var user_id=$('input[name=user_id]').val()
								var joinCode=$('input[name=join_code]').val()
								var str = user_id+','+join_code+','+store_code
								window.webkit.messageHandlers.gotoQK.postMessage(str)
							})
						}
						//等待时间
			...
			$('.weui-actionsheet__cell').click(function(){
				$('#showIOSActionSheet').attr('disabled','disabled')
				...
				util.post({
					url: api_www+'/v5.Approval/changeState',
					data: {
						token: token,
						join_code: join_code,
						type:type,
...
// ios
			$(function() {
				...
				var $iosMask = $('#iosMask');
				var $iosActionsheett = $('#iosActionsheett');
				var $iosMaskt = $('#iosMaskt');

				function hideActionSheet() {
					$iosActionsheet.removeClass('weui-actionsheet_toggle');
					$iosMask.fadeOut(200);
					$iosActionsheett.removeClass('weui-actionsheet_toggle');
					$iosMaskt.fadeOut(200);
				}

				$iosMask.on('click', hideActionSheet);
				$iosMaskt.on('click', hideActionSheet);
				$('#iosActionsheetCancel').on('click', hideActionSheet);
				$('#iosActionsheetCancelt').on('click', hideActionSheet);
				$("#showIOSActionSheet").on("click", function() {
					$iosActionsheet.addClass('weui-actionsheet_toggle');
					$iosMask.fadeIn(200);
				});
				$("#showIOSActionSheett").on("click", function() {
					$iosActionsheett.addClass('weui-actionsheet_toggle');
					$iosMaskt.fadeIn(200);
				});
			...
			var isAndroid = u.indexOf('Android') > -1 || u.indexOf('Linux') > -1; //android终端或者uc浏览器   
			var isiOS = !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端   
			var token, join_code, function_id, fram_id, id, time, account
			if(isAndroid) {				
				$('.bback').click(function(){
					window.xmh.finish()
				})
				$('.ae,.ll_fh').click(function(){
					window.xmh.finish()
				})
				
			} else {				
				$('.bback').click(function(){
					window.webkit.messageHandlers.pop.postMessage('0');
				})
				//返回
				$('.ae,.ll_fh').click(function(){
					window.webkit.messageHandlers.approveGoBack.postMessage('0')				
				})
			}